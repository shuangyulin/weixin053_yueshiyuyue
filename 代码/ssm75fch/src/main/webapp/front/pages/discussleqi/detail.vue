...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discussleqi');
			...
			onAcrossTap(tableName){
     			uni.setStorageSync('crossTable','discussleqi');
				uni.setStorageSync(`crossObj`, this.detail);
				this.$utils.jump(`../${tableName}/add-or-update?cross=true`);
			},
			// 获取详情
			async init(){
				let res = await this.$api.info('discussleqi', this.id);
			...
			async upCallback(mescroll) {
				                let res = await this.$api.list('discussdiscussleqi', {
					page: mescroll.num,
					limit: mescroll.size,
					refid: this.id
				});
			...
			// 添加评论
			onCommentTap() {
				this.$utils.jump(`../discussdiscussleqi/add-or-update?refid=${this.id}`)
			},
			// 点赞
			async zan() {
				let _this = this;
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							...
							await _this.$api.update('discussleqi', _this.detail);
			...
			async onFinishTap() {
				...
				await this.$api.update('discussleqi', this.detail);
				this.$utils.msg('审核成功');
